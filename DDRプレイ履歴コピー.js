...
(function() {
    ...
    add_button(document.getElementsByTagName("h2")[0], function (e) {
        ...
        $('table.table-ui tbody tr').each(function(rownum) {
            ...
            // ランク
            const rankSrc = $(this).find('img[src^="/game/ddr/ddrworld/images/playdata/rank_"]').attr('src');
            const rank = rankSrc.split('/').pop().split('?')[0].replace(/rank_s_|\.png/g, "").replace("_p", "+").replace("_m", "-").toUpperCase();
            // 日時
        ...
        if (confirm("送信しますか？\nいいえの場合ダイアログ表示のみ行って終了します。")) {
            ...
            GM_xmlhttpRequest({
                ...
                ontimeout: res => {
                    alert("[timeout] status: " + res.status + "\n" + res.responseText);
                }
            });
        } else {
            // ダイアログ表示
